...
use project::{Project, RealFs, search::SearchQuery};
    ...
    Application::headless().run(|cx| {
        ...
        project.clone().update(cx, move |_, cx| {
            cx.spawn(async move |_, cx| {
                ...
                let matches = project.update(cx, |this, cx| this.search(query, cx)).unwrap();
                while let Ok(m) = matches.recv().await {
                    if first_match.is_none() {
                        first_match = Some(timer.elapsed());
                    }

                }
                let elapsed = timer.elapsed();
                println!("Finished project search after {elapsed:?}, first match found in {first_match:?}");
                drop(project);
